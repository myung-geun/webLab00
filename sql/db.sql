...
ALTER TABLE students CHANGE COLUMN major major VARCHAR(40) NOT NULL;
...
UPDATE students SET dept_no=6 WHERE name='Chloe';
...
select * from students where year<3;
select * from students s,department d where s.major='Business Administration' and s.dept_no=d.dept_no;
SELECT * FROM students WHERE student_id LIKE "2007%";
SELECT * FROM students ORDER BY student_id;
SELECT major,avg(year) as AVG_year FROM students GROUP BY major HAVING AVG_year>3;
SELECT * FROM students WHERE student_id LIKE "2007%" LIMIT 2;


CREATE database imdb;
use imdb;
source /Users/jimyeong-geun/Documents/MAMP/htdocs/db/imdb_small.sql;
...
select first_name,last_name from actors a join roles r1 on r1.actor_id = a.id
...
use simpsons;
source /Users/jimyeong-geun/Documents/MAMP/htdocs/db/simpsons.sql;


SELECT grade FROM grades g,courses c,students s WHERE  g.student_id=s.id and g.course_id=c.id and c.name='Computer Science 143';
SELECT s.name,grade FROM grades g,courses c,students s WHERE  g.student_id=s.id and g.course_id=c.id and c.name='Computer Science 143' and g.grade<= 'B-';
SELECT  s.name FROM grades g,courses c,students s WHERE  g.student_id=s.id and g.course_id=c.id and g.grade<='B-';
SELECT DISTINCT c.name FROM grades g,courses c,students s WHERE  g.student_id=s.id and g.course_id=c.id GROUP BY c.name HAVING count(s.id) >= 2;

use imdb;

select name from movies where year = 1995;
select count(*) people from movies m join roles on m.id = roles.movie_id where m.name = "Lost in Translation";
select a.first_name, a.last_name, r.role from movies m join roles r on m.id = r.movie_id join actors a on a.id = r.actor_id where m.name = "Lost in Translation";
select d.first_name, d.last_name, m.name from movies m join movies_directors md on m.id = md.movie_id join directors d on d.id = md.director_id where m.name = "Fight Club";
select count(*) maked from movies m join movies_directors md on m.id = md.movie_id join directors d on d.id = md.director_id where d.first_name = "Clint" and d.last_name = "Eastwood";
select m.name movie_name, d.first_name, d.last_name from movies m join movies_directors md on m.id = md.movie_id join directors d on d.id = md.director_id where d.first_name = "Clint" and d.last_name = "Eastwood";
select d.first_name, d.last_name from movies_genres mg join movies_directors md on mg.movie_id = md.movie_id join directors d on d.id = md.director_id where mg.genre = "Horror";
select a.first_name, a.last_name from movies m join movies_directors md on m.id = md.movie_id join directors d on d.id = md.director_id join roles r on r.movie_id = m.id join actors a on a.id = r.actor_id where d.first_name = "Christopher" and d.last_name = "Nolan";